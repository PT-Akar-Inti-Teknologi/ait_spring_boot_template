...
import org.ait.project.guideline.example.shared.dto.template.ResponseCollectionContent;
...
public class ResponseHelper {
  ...
  public <T> ResponseEntity<ResponseTemplate<ResponseDetail<T>>> createResponseDetail(
    ...
    return ResponseEntity.status(responseEnum.getHttpStatus())
        .body(
            new ResponseTemplate<>(responseMessageHelper.getResponseSchema(responseEnum),
                new ResponseDetail<>(body))
        );
  }
  ...
  public <P, T> ResponseEntity<ResponseTemplate<ResponseCollection<T>>> createResponseCollection(
    ...
    return ResponseEntity.status(responseEnum.getHttpStatus())
        .body(
            new ResponseTemplate<>(responseMessageHelper.getResponseSchema(responseEnum),
                createCollection(page, contents))
        );
  }
  ...
  public Object createResponseErrorTemplate(ResponseEnum invalidParam,
    ...
    return new ResponseTemplate<>(responseMessageHelper.getResponseSchema(invalidParam),
        responseError);
  }
}
